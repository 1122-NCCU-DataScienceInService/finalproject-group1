...
* **XGBoost 具有下列優點**:
  ...
  * **強大的正則化**：XGBoost 支持 L1 和 L2 正則化，這有助於防止過度擬合，提高模型的泛化能力。
...
**k-fold cross validation**
  ...
  * `max_depth`：樹的最大深度限制，此參數控制每棵決策樹的深度，**避免過度擬合**，`max_depth = 6`
...
```
  ...
  1. **高AUC評分**：第 30 次反覆運算的 AUC 評分是整個反覆運算過程中最高的，達到了 0.841223

  2. **合理的標準差**：標準差 0.01424375 相對較小，表明模型的性能在不同的交叉驗證折次中較為穩定

  
    相比之下，後續的反覆運算雖然也達到了類似的 AUC 評分，但並沒有顯著超過第 30 次反覆運算的 AUC 評分，
  ...
  *第 31 次反覆運算的 test-auc 是 0.8411549	，比第 30 次反覆運算低。*

  *第 32 次反覆運算的 test-auc 是 0.8407787，也比第 31 次反覆運算低。*

  *此外，在第 30 次反覆運算之後，儘管訓練集上的 AUC 評分（train-auc）不斷上升，*

  *但測試集上的 AUC 評分並沒有明顯改善，甚至有所波動，這可能表明模型開始**過擬合**訓練數據。*
