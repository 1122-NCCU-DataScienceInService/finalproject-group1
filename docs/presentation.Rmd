...
```
  ...
  1. **高AUC評分**：第 31 次反覆運算的 AUC 評分是整個反覆運算過程中最高的，達到了 0.8439923

  2. **合理的標準差**：標準差 0.006611932 相對較小，表明模型的性能在不同的交叉驗證折次中較為穩定

  
    相比之下，後續的反覆運算雖然也達到了類似的 AUC 評分，但並沒有顯著超過第 31 次反覆運算的 AUC 評分，
  ...
  *第 32 次反覆運算的 test-auc 是 0.8438350	，比第 31 次反覆運算低。*

  *此外，在第 31 次反覆運算之後，儘管訓練集上的 AUC 評分（train-auc）不斷上升，*
...
```
  ...
  ***檢定統計量 Z = 42.437  表示兩個模型的 AUC 差異較大
  ***p =2.2e-16，這是一個非常小的值，表明 AUC 之間的差異具有統計學意義。通常，p 值小於 0.05 被認為是顯著的，因此這個結果非常顯著
  ***- AUC 差異的 95% 信賴區間為 [0.3658094, 0.4012353]。這意味著我們有 95% 的信心認為真實的 AUC 差異在這個區間內。由於該區間不包含 0，這進一步支持 AUC 之間存在顯著差異的結論。
  ***DeLong 檢定表明，兩條ROC曲線的 AUC 之間存在統計學上顯著的差異。XGBoost顯著優於null model，這從高Z值、極低的p-value 以及 AUC 差異的信賴區間可以看出
